    ...
    <div class="max-w-xl">
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h2 class="text-sm font-semibold text-gray-900">Form Edit Cabang</h2>
                @if (!$cabang->is_active)
                    <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">Nonaktif</span>
                @endif
            </div>

            <form method="POST" action="{{ route('admin.cabang.update', $cabang) }}" class="px-6 py-5 space-y-4">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="alamat" name="alamat" rows="3"
                        class="w-full px-3 py-2 border rounded-lg text-sm
                               focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent
                               {{ $errors->has('alamat') ? 'border-red-400 bg-red-50' : 'border-gray-300' }}">{{ old('alamat', $cabang->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
