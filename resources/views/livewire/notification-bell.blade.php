<div class="relative" x-data="{ open: @entangle('isOpen') }" wire:poll.{{ $pollInterval }}s="$refresh">
    ...
    <button type="button" wire:click="toggleOpen"
        class="relative flex items-center justify-center w-9 h-9 rounded-lg
               text-[--color-text-muted] hover:bg-[--color-bg-subtle] hover:text-[--color-text-secondary] transition-colors">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-150"
        ...
        class="absolute right-0 top-full mt-2 w-96 bg-white rounded-xl shadow-xl
                border border-[--color-border-default] overflow-hidden z-50"
        style="display: none;">

        {{-- Header --}}
        <div class="px-4 py-3 border-b border-[--color-border-default] flex items-center justify-between">
            <div class="flex items-center gap-2">
                <h3 class="text-sm font-semibold text-[--color-text-primary]">Notifikasi</h3>
                @if ($this->unreadCount > 0)
        ...
        <div class="max-h-96 overflow-y-auto divide-y divide-gray-50">
            @forelse($this->recentNotifications as $notif)
                ...
                <div
                    class="flex items-start gap-3 px-4 py-3 {{ $isUnread ? 'bg-indigo-50/50' : '' }}
                            hover:bg-[--color-bg-subtle] transition-colors duration-100 group">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-xs font-semibold text-[--color-text-primary] leading-snug">
                            {{ $data['title'] ?? '' }}
                            @if ($isUnread)
                                <span
                        ...
                        <p class="text-xs text-[--color-text-secondary] mt-0.5 leading-snug line-clamp-2">
                            {{ $data['message'] ?? '' }}
                        </p>
                        <p class="text-xs text-[--color-text-disabled] mt-1">
            ...
            @empty
                <div class="px-4 py-12 text-center">
                    <div class="w-12 h-12 bg-[--color-bg-inset] rounded-2xl flex items-center justify-center mx-auto mb-3">
                        <svg class="w-6 h-6 text-[--color-text-muted]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                        </svg>
                    </div>
                    <p class="text-sm font-semibold text-[--color-text-secondary]">Tidak ada notifikasi</p>
                </div>
            @endforelse
        </div>
        ...
        @if ($this->recentNotifications->isNotEmpty())
            <div class="px-4 py-2.5 border-t border-[--color-border-default] bg-[--color-bg-subtle] text-center">
